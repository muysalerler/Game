...
static void look(Room *room, Player *player);
...
void gameRun(Game *g)
{
    ...
    {
        printf("\n> ");
        if (!fgets(command, MAX_DESC, stdin))
            break;
        command[strcspn(command, "\n")] = 0;

        if (strcmp(command, "look") == 0)
        {
            look(g->player.currentRoom, &g->player);
        }
        ...
        else if (strcmp(command, "use miraclefruit") == 0)
        {
            useMiracleFruit(&g->player);
        }
        else if (strncmp(command, "drop ", 5) == 0)
        {
            dropItem(&g->player, g->player.currentRoom, command + 5);
        }
...
static void look(Room *room, Player *player)
{
    ...
    if (!room->eventTriggered)
    {
        ...
        {
        case EVENT_GOOD_1:
            printf("You see an angelic figure, it heals you (+10 health)!\n");
            player->health += 10;
            break;
        case EVENT_GOOD_2:
            printf("A friendly spirit bolsters your shield (+5 shield)!\n");
            player->shield += 5;
            break;
        case EVENT_GOOD_3:
            printf("A calm aura grants you strength (+5 strength)!\n");
            player->strength += 5;
            break;
        case EVENT_GOOD_4:
            printf("You find a hidden tonic that restores 10 health.\n");
            player->health += 10;
            break;
        case EVENT_GOOD_5:
            printf("A gentle breeze whispers encouragement (+5 strength).\n");
            player->strength += 5;
            break;
        case EVENT_BAD_1:
            printf("There are many snakes here, one bites you (-10 health)!\n");
            player->health -= 10;
            break;
        case EVENT_BAD_2:
            printf("A malicious spirit drains your shield (-5 shield)!\n");
            if (player->shield > 5)
                player->shield -= 5;
            else
                player->shield = 0;
            break;
        case EVENT_BAD_3:
            printf("A dreadful presence weakens you (-5 strength).\n");
            ...
            break;
        case EVENT_BAD_4:
            printf("Toxic fumes fill the air, you lose 10 health.\n");
            player->health -= 10;
            break;
        case EVENT_BAD_5:
            printf("A cursed whisper saps your strength (-5 strength).\n");
            if (player->strength > 5)
                player->strength -= 5;
            else
                player->strength = 0;
